...
const Settings = () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                Notification.error({
                    ...
                    content: __("Error saving settings. Please try again.", "mos-faqs"),
                    duration: 3,
                    position: 'topRight',
                });
            }
    ...
    const handleReset = async (section) => {
        try {
            const result = await apiFetch({
                path: "/mos-faqs/v1/options/reset-settings",
                method: 'POST',
                data: { name: section }
            });
            console.log(result);
            if (result.success) {
                ...
                Notification.error({
                    title: __("Error", "mos-faqs"),
                    content: __("Error resetting settings. Please try again.", "mos-faqs"),
                    duration: 3,
                    position: 'topRight',
                });
            }
    ...
    useEffect(() => {
        if (mos_faqs_ajax_obj?.extraMenuItems) {
            setRemoteItems(mos_faqs_ajax_obj.extraMenuItems);
        }
    }, []);
        ...
        <>
            ...
            <Nav 
                items= {[
                    {
                        itemKey: "vip",
                        text: __("VIP Priority Support", "mos-faqs"),
                        icon: <IconSend />,
                        link: "https://wordpress.org/support/plugin/mos-faqs/",
                        linkOptions: {
                            target: '_blank',
                    ...
                    {
                        itemKey: "help",
                        text: __("Help Center", "mos-faqs"),
                        icon: <IconHelpCircle />,
                        link: "https://mostak-shahid.github.io/plugins/mos-faqs.html",
                        linkOptions: {
                            target: '_blank',
                            rel: 'noopener noreferrer', // recommended for security
                        },
                    ...
                    {
                        itemKey: "community",
                        text: __("Join the Community", "mos-faqs"),
                        icon: <IconUserAdd />,
                    ...
                    {
                        itemKey: "rate",
                        text: __("Rate Us", "mos-faqs"),
                        icon: <IconLikeHeart />,
                        link: "https://wordpress.org/support/plugin/mos-faqs/reviews/",
                        linkOptions: {
                            target: '_blank',
                            rel: 'noopener noreferrer', // recommended for security
                        },
