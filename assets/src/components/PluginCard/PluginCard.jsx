...
export default function PluginCard(plugin) {
    ...
    */
    ...
    const getButtonLabel = () => {
        switch (pluginStatus) {
            case "checking":
                return __("Checking...", "mos-faqs");
            case "not_installed":
                return __("Install Now", "mos-faqs");
            case "installed":
                return __("Activate", "mos-faqs");
            case "installing":
                return __("Installing...", "mos-faqs");
            case "installation_complete": // New state
                return __("Installed", "mos-faqs");
            case "activating":
                return __("Activating...", "mos-faqs");
            case "activated":
                return __("Activated", "mos-faqs");
            case "error":
                return __("Try Again", "mos-faqs");
            default:
                return __("Install Now", "mos-faqs");
        }
    };
    ...
    const installPlugin = async () => {
        ...
        setErrorMessage("");
        try {
            await formDataPost('mos_faqs_ajax_install_plugins', {
                sub_action:'install',
                download_url:download_url,                
                plugin_slug:plugin_slug,
                plugin_file:plugin_file,
    ...
    return (
        <div className="mos-faqs-plugin-card p-4">
            <Space align='center'>
                <img
                    alt={name}
                    src={image}
            ...
            <div className="flex justify-between mt-1">
                <Space align='center'><IconHistogram style={{fontSize: 24}} /><span>{__(`${active_installs} ${active_installs>0?"+":""} active installations`, "mos-faqs")}</span></Space>
                <Space align='center'><WordPress width='24' height='24'/> {__(`Tested with ${tested}`, "mos-faqs")}</Space>
            </div>
